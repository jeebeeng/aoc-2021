...
impl Bingo {
    pub fn new(lines: &str) -> Bingo {
        ...
        Bingo { drawn, boards, curr_drawn: 0 }
    }
    ...
    fn check_boards(&self) -> Vec<Vec<u8>> {
        let winner: Vec<Vec<u8>> = Vec::new();
    ...
    pub fn draw(&mut self) -> Vec<Vec<u8>> {
        self.curr_drawn += 1;
        self.check_boards()
    }
}
...
fn part_one(lines: &str) -> u32 {
    ...
    let mut winner: Vec<Vec<u8>> = Vec::new();

    while winner.is_empty() {
        winner = bingo.draw();
    }

    score(&winner, &bingo.drawn[0..bingo.curr_drawn as usize])
}
